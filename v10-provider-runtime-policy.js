const MARK = Symbol.for("aiguka.v10.providerRuntimePolicy.v1");
...
export function installProviderRuntimePolicy() {
  ...
  globalThis.fetch = async function providerPolicyFetch(input, init = {}) {
    ...
    const sorted = rows
      .filter(ready)
      .sort((a, b) => priority(a) - priority(b) || String(a.provider_key || "").localeCompare(String(b.provider_key || "")));
  ...
  globalThis[MARK] = { version: "v1", source: "/ai-providers", priority: "settings.runtime_order", productionGate: "smoke_test" };
  console.log("[AIGUKA V10] provider runtime policy enabled: /ai-providers priority and smoke-test gate are authoritative");
