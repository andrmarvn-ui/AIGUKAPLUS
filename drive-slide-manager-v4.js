...
export function installDriveSlideManagerV4(app, { supabaseUrl, publishableKey, serviceRoleKey }) {
  ...
  const encryptionKey = crypto.createHash("sha256").update(`${serviceRoleKey || dbKey}|${supabaseUrl}|AIGUKA_GOOGLE_DRIVE_OAUTH_V1`).digest();
  const encrypt = (value) => {
  ...
  router.post("/test-slide", async (req, res) => {
    try {
      ...
      // AIGUKA_MESSENGER_GENERIC_CAROUSEL_V1
