...
begin
  ...
  with message_base as (
    ...
    select
      ...
      'v9:'||ng.page_id||':'||ng.customer_id||':'||to_char(ng.conversation_started_at,'YYYYMMDDHH24MISS') as conversation_id,
      ...
      ng.message_count,
      coalesce(nullif(ll.last_snippet,''),ng.message_count||' tin khách · nội dung hội thoại được bảo mật') as last_snippet,
      coalesce(ng.source_system,'v9_core') as source_channel,
      'v9_normalized_reporting'::text as identity_source
    ...
    where (nullif(btrim(p_page_id),'') is null or u.page_id=p_page_id)
      ...
      and (
        ...
        or concat_ws(' ',u.customer_name,u.phone,u.zalo,u.sender_id,u.customer_id,u.conversation_id,
          u.ad_name,u.campaign_name,u.adset_name,u.product_group,u.product_label,u.last_snippet,u.pancake_employee)
  ...
  select jsonb_build_object(
    ...
    'source','v9_normalized_reporting_plus_legacy'
  ) into v_result;
  return v_result;
end;
$function$;
