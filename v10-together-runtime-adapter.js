const MARK = Symbol.for("aiguka.v10.togetherRuntimeAdapter.v1");

function isTogetherResponsesUrl(input) {
  try {
    const value = input instanceof Request ? input.url : String(input);
    const url = new URL(value);
    return url.hostname.toLowerCase() === "api.together.ai" && /\/responses\/?$/i.test(url.pathname);
  } catch {
    return false;
  }
}
...
function maxTokens() {
  const parsed = Number(process.env.AIGUKA_TOGETHER_MAX_TOKENS || 1200);
...
export function installTogetherRuntimeAdapter() {
  if (globalThis[MARK]) return globalThis[MARK];
  const nativeFetch = globalThis.fetch.bind(globalThis);

  globalThis.fetch = async function togetherFetch(input, init = {}) {
    if (!isTogetherResponsesUrl(input)) return nativeFetch(input, init);
    ...
    const response = await nativeFetch("https://api.together.ai/v1/chat/completions", {
      ...init,
      headers,
  ...
  globalThis[MARK] = {
    ...
    reasoning: "disabled_for_customer_latency",
    toolChoice: "required",
  };
  console.log("[AIGUKA V10] Together AI chat-completions adapter v1 enabled");
  return globalThis[MARK];
}

installTogetherRuntimeAdapter();
