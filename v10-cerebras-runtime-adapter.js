const MARK = Symbol.for("aiguka.v10.cerebrasRuntimeAdapter.v1");
const DEFAULT_MAX_TOKENS = 1200;

function isCerebrasResponsesUrl(input) {
  try {
    const value = input instanceof Request ? input.url : String(input);
    const url = new URL(value);
    return url.hostname.toLowerCase() === "api.cerebras.ai" && /\/responses\/?$/i.test(url.pathname);
  } catch {
    return false;
  }
}
  ...
  return (body.tools || [])
    ...
    .map((tool) => ({
      ...
      function: {
        ...
        parameters: tool.parameters || { type: "object", properties: {}, required: [], additionalProperties: false },
...
function maxTokens() {
  const parsed = Number(process.env.AIGUKA_CEREBRAS_MAX_TOKENS || DEFAULT_MAX_TOKENS);
  if (!Number.isFinite(parsed)) return DEFAULT_MAX_TOKENS;
  return Math.max(256, Math.min(4000, Math.floor(parsed)));
}

function toCerebrasBody(body = {}) {
  const tools = toolsFromResponses(body);
  const toolChoice = normalizeToolChoice(body.tool_choice) || (tools.length ? "required" : undefined);
  return {
    model: body.model,
    ...
    ...(typeof body.parallel_tool_calls === "boolean" ? { parallel_tool_calls: body.parallel_tool_calls } : { parallel_tool_calls: false }),
  };
}
...
export function installCerebrasRuntimeAdapter() {
  if (globalThis[MARK]) return globalThis[MARK];
  const nativeFetch = globalThis.fetch.bind(globalThis);

  globalThis.fetch = async function cerebrasFetch(input, init = {}) {
    if (!isCerebrasResponsesUrl(input)) return nativeFetch(input, init);
    ...
    headers.set("X-Cerebras-Version-Patch", "2");

    const response = await nativeFetch("https://api.cerebras.ai/v1/chat/completions", {
      ...init,
      headers,
      body: JSON.stringify(toCerebrasBody(body)),
    });
  ...
  console.log("[AIGUKA V10] Cerebras native chat-completions adapter v1 enabled");
  return globalThis[MARK];
}

installCerebrasRuntimeAdapter();
