...
function publicRequest(req) {
  ...
  if (["/health", "/favicon.ico", "/robots.txt", "/__aiguka/verify-meta-signature", "/auth/login", "/auth/logout"].includes(path)) return true;
  return ["/functions/v1/aiguka-v9-webhook", "/functions/v1/aiguka-v8-webhook"]
...
function internalRequest(req) {
  ...
  const supplied = Buffer.from(String(req.headers["x-aiguka-internal-auth"] || ""));
  const expected = Buffer.from(INTERNAL_TOKEN);
  return loopback && supplied.length === expected.length && crypto.timingSafeEqual(supplied, expected);
}

function cookieOptions(req, maxAgeSeconds) {
  const secure = process.env.NODE_ENV === "production"
    || String(req.headers["x-forwarded-proto"] || "").split(",", 1)[0].trim() === "https";
  return { httpOnly: true, secure, sameSite: "lax", path: "/", maxAge: Math.max(0, Number(maxAgeSeconds || 0)) * 1000 };
}
...
function saveSession(res, req, session) {
  res.cookie(ACCESS_COOKIE, String(session.access_token || ""), cookieOptions(req, Number(session.expires_in || DEFAULT_ACCESS_TTL_SECONDS)));
  res.cookie(REFRESH_COOKIE, String(session.refresh_token || ""), cookieOptions(req, DEFAULT_REFRESH_TTL_SECONDS));
}

function localOwnerId() {
  return String(process.env.AIGUKA_LOCAL_ADMIN_ID || process.env.AIGUKA_LOCAL_ADMIN_EMAIL || "").trim().toLowerCase();
}

function localOwnerConfigured() {
  return Boolean(localOwnerId()
...
function verifyLocalOwnerPassword(login, password) {
  if (!localOwnerConfigured()) return false;
  if (String(login || "").trim().toLowerCase() !== localOwnerId()) return false;
  const [scheme, saltHex, digestHex] = String(process.env.AIGUKA_LOCAL_ADMIN_PASSWORD_SCRYPT || "").split("$");
...
function localOwnerCookieValue(login) {
  const encoded = Buffer.from(String(login || "").trim().toLowerCase(), "utf8").toString("base64url");
  const signature = crypto.createHmac("sha256", String(process.env.AIGUKA_LOCAL_ADMIN_SESSION_SECRET || ""))
...
function verifyLocalOwnerCookie(value) {
  ...
  let login = "";
  try { login = Buffer.from(encoded, "base64url").toString("utf8").trim().toLowerCase(); }
  catch { return null; }
  if (login !== localOwnerId()) return null;
  const expectedSignature = crypto.createHmac("sha256", String(process.env.AIGUKA_LOCAL_ADMIN_SESSION_SECRET || ""))
    .update(encoded).digest("base64url");
  return safeEqualBuffer(Buffer.from(suppliedSignature), Buffer.from(expectedSignature)) ? login : null;
}

function saveLocalOwnerSession(res, req, login) {
  res.cookie(LOCAL_OWNER_COOKIE, localOwnerCookieValue(login), cookieOptions(req, DEFAULT_REFRESH_TTL_SECONDS));
  res.clearCookie(ACCESS_COOKIE, cookieOptions(req, 0));
  res.clearCookie(REFRESH_COOKIE, cookieOptions(req, 0));
}
...
async function authRequest(supabaseUrl, publishableKey, path, request = {}) {
  const response = await fetch(`${supabaseUrl}${path}`, {
    ...request,
    headers: { apikey: publishableKey, "content-type": "application/json", ...(request.headers || {}) },
...
export function installSupabaseAdminAuth(app, options = {}) {
  ...
  const publishableKey = String(options.publishableKey || process.env.SUPABASE_PUBLISHABLE_KEY || process.env.SUPABASE_ANON_KEY || "");
  ...
  app.post("/auth/login", form, async (req, res) => {
    ...
    const submittedLogin = String(req.body?.email || "").trim();
    const submittedPassword = String(req.body?.password || "");

    if (verifyLocalOwnerPassword(submittedLogin, submittedPassword)) {
      saveLocalOwnerSession(res, req, submittedLogin);
    ...
    try {
      const result = await authRequest(supabaseUrl, publishableKey, "/auth/v1/token?grant_type=password", {
        method: "POST", body: JSON.stringify({ email: submittedLogin, password: submittedPassword }),
      });
      if (!result.response.ok || !result.data?.access_token) {
        res.status(401).type("html").send(loginHtml(next, "ID/email hoặc mật khẩu không đúng."));
        return;
      }
  ...
  app.use(async (req, res, next) => {
    ...
    const cookies = parseCookies(req.headers.cookie);
    const localLogin = verifyLocalOwnerCookie(cookies[LOCAL_OWNER_COOKIE]);
    if (localLogin) {
      req.aigukaUser = { id: "local-owner", email: localLogin, app_metadata: { role: "owner" } };
  ...
  app.get("/auth/session", (req, res) => {
    res.setHeader("cache-control", "no-store");
    res.status(200).json({ ok: true, user: { id: req.aigukaUser.id, email: req.aigukaUser.email } });
  });
}

export const __private__ = {
  ACCESS_COOKIE, REFRESH_COOKIE, LOCAL_OWNER_COOKIE, authorizedUser, internalRequest,
  parseCookies, publicRequest, safeNext, verifyLocalOwnerCookie, verifyLocalOwnerPassword,
};
