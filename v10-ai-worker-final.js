...
function unsupportedPriceReply(reply, modelInput) {
  ...
  const knowledge = verifiedKnowledgeText(modelInput).replace(/[\s.,/_+–—-]+/g, "");
  return priceMatches.some(function (match) {
    const normalized = qualityNormalize(match).replace(/[\s.,/_+–—-]+/g, "");
...
async function sovereignProviderDecision(provider, providerModelInput, validationInput = providerModelInput) {
    ...
    if (!violations.length) {
      ...
      if (protectedViolations.length) {
        ...
        return {
          ...
          commerceRepairApplied: protectedDecision?.commerce_integrity?.repair_mode !== "verified_passthrough",
          hardRepair: false,
        };
      }
    }
  ...
  // A provider that fails the same hard policy twice is not allowed to drop the
  // customer or send its text. The deterministic guard replaces the unsafe business
  ...
  try {
    ...
    if (!remaining.length) {
      return {
        ...
        rawDecision: firstRawDecision,
        finalRawDecision,
        validationFeedbackRounds: 2,
        validationFeedback: feedback,
        commerceRepairApplied: true,
        hardRepair: true,
        hardRepairReason: feedback.join("|").slice(0, 600),
      };
    }
  } catch {}
...
async function processOne(row, availableProviders, knowledgeSnapshot) {
  ...
  const validationInput = {
    ...
    authority: {
      ...
      mapping_and_catalog_are_grounding_evidence: true,
      validators_may_replace_unsafe_business_output: true,
    ...
    await core("v9_decisions?id=eq." + claimed.id + "&status=eq.shadow_ai_processing", {
      ...
      body: {
        ...
        output: {
          ...
          hard_commerce_policies_applied: true,
          validator_version: "v10_hard_commerce_integrity_v2",
          validator_rewrites_business_output: Boolean(result.commerceRepairApplied),
