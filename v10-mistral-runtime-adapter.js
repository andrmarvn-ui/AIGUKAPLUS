const MARK = Symbol.for("aiguka.v10.mistralRuntimeAdapter.v1");
const DEFAULT_MAX_TOKENS = 1200;

function isMistralResponsesUrl(input) {
  try {
    const value = input instanceof Request ? input.url : String(input);
    const url = new URL(value);
    return url.hostname.toLowerCase() === "api.mistral.ai" && /\/responses\/?$/i.test(url.pathname);
  } catch {
    return false;
  }
}
...
function normalizeToolChoice(choice) {
  ...
  if (["none", "auto", "any", "required"].includes(choice)) return choice;
...
function maxTokens() {
  const parsed = Number(process.env.AIGUKA_MISTRAL_MAX_TOKENS || DEFAULT_MAX_TOKENS);
  if (!Number.isFinite(parsed)) return DEFAULT_MAX_TOKENS;
  return Math.max(256, Math.min(4000, Math.floor(parsed)));
}

function toMistralBody(body = {}) {
...
export function installMistralRuntimeAdapter() {
  if (globalThis[MARK]) return globalThis[MARK];
  const nativeFetch = globalThis.fetch.bind(globalThis);

  globalThis.fetch = async function mistralFetch(input, init = {}) {
    if (!isMistralResponsesUrl(input)) return nativeFetch(input, init);
    ...
    const response = await nativeFetch("https://api.mistral.ai/v1/chat/completions", {
      ...init,
      headers,
  ...
  globalThis[MARK] = {
    ...
    toolChoice: "required",
    strictFieldForwarded: false,
  };
  console.log("[AIGUKA V10] Mistral native chat-completions adapter v1 enabled");
  return globalThis[MARK];
}

installMistralRuntimeAdapter();
