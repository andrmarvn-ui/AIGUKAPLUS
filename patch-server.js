...
const existingBlock = /installReportRoutes\(app,[\s\S]*?(?:installStableV7Dashboard\(app\);|pageRoutes\.set\("\/tich-hop-ai","aiguka-v8-ai-provider-ui"\);)/;
if (existingBlock.test(source)) source = source.replace(existingBlock, routeInstall);
else if (!source.includes("installReportRoutes(app")) {
